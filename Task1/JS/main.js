...
function Add(){
    if(inputTask.value.length>0 && inputTask.value.trim()!==""){
        ...
        let newTaskElement="<li class='list-group-item items'><i class='bi bi-trash-fill'></i> "+tasks.data+" <span class='date'>"+tasks.date+" </span></li>";
...
document.addEventListener("click",function(e){
    if(e.target.hasAttribute("class") && e.target.getAttribute("class").includes("bi-trash-fill")){
        e.target.parentNode.remove()
    }
    if(document.querySelector(".bi-trash-fill")==undefined){
        deleteAllTasks.classList.add("d-none");
    }
})